-- Harden active SECURITY DEFINER RPCs that are still used by the app.
-- They remain callable by authenticated users, but can only operate on the
-- caller's own user id. Server-side calls with the service_role JWT remain
-- allowed for Edge Functions that already enforce their own user context.
...
revoke execute on function public.get_financial_metrics(
  uuid,
  date,
  date
) from public, anon;
grant execute on function public.get_financial_metrics(
  uuid,
  date,
  date
) to authenticated, service_role;
