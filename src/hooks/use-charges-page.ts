...
export function mapInvoiceToChargeRow(invoice: Invoice): ChargeRow {
  ...
  const chargeType = getNeurofinanceChargeType(invoice);
  const feeCents = Number(raw.actual_fee_amount_cents ?? raw.estimated_fee_amount_cents ?? raw.fee_amount_cents ?? raw.total_fee_cents);
  const netCents = Number(raw.net_amount_cents ?? raw.net_value_cents);
  const grossAmount = Number(invoice.amount || 0);
  const feeAmount = Number.isFinite(feeCents) ? Math.abs(feeCents) / 100 : null;
  const netAmount = Number.isFinite(netCents)
  ...
  return {
    ...
    chargeType,
    origin: raw.appointment_id ? "appointment" : String(raw.origin || raw.channel || "neurofinance"),
    feeAmount,
    netAmount,
    fundsStatus: raw.funds_status || null,
    availableAt: normalizeDateKey(raw.available_at || raw.estimated_credit_at),
    ...
    links: {
      ...
      receiptUrl: raw.receipt_url || raw.provider_receipt_url || null,
      invoiceUrl: raw.invoice_url || raw.pdf_url || raw.bank_slip_url || null,
      nfseUrl: raw.nfse_url || raw.fiscal_document_url || null,
    },
...
export function filterChargeRows(rows: ChargeRow[], params: Omit<ChargesPageParams, "scope" | "page" | "pageSize">) {
  ...
  return rows.filter((row) => {
    ...
    const matchesType = typeFilter.size === 0 || typeFilter.has(row.chargeType);
