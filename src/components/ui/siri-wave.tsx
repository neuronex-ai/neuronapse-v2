...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float alpha = smoothstep(0.01, 0.22, peak);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float peak = max(max(col.r, col.g), col.b);
    float alpha = smoothstep(0.012, 0.2, peak);
    fragColor = vec4(col, alpha);
}
void main(){ mainImage(gl_FragColor, gl_FragCoord.xy); }`;
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    const compile = (type: number, src: string) => {
      ...
      if (!shader) throw new Error("shader allocation error");
    ...
    const dim = Math.max(1, Math.round(size * renderScale * pixelRatio));
    canvas.width = dim;
    canvas.height = dim;
    ...
    const draw = (now: number) => {
      ...
      gl.uniform1f(uTime, reduceMotion ? 0.9 : (now - start) / 1000);
  ...
  return (
    <canvas
      ...
      aria-hidden="true"
      className={cn(
        "block bg-transparent transition-[filter,opacity] duration-300 motion-reduce:transition-none",
        "brightness-[0.46] contrast-125 saturate-110 dark:brightness-100 dark:contrast-110 dark:saturate-100",
        className,
      )}
