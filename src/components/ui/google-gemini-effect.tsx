...
type GeminiPathLengths = readonly [
  MotionValue<number>,
  MotionValue<number>,
  MotionValue<number>,
  MotionValue<number>,
...
const PATH_OPACITY = [0.58, 0.72, 0.94, 0.72, 0.58] as const;

export function GoogleGeminiEffect({
  pathLengths,
  className,
}: {
  pathLengths: GeminiPathLengths;
  className?: string;
}) {
  const uid = useId().replace(/:/gu, "");
  const blurId = `neuronex-gemini-blur-${uid}`;
  const gradientId = `neuronex-gemini-gradient-${uid}`;

  return (
    <div
      aria-hidden="true"
      className={cn("relative h-full w-full text-foreground", className)}
    >
      <svg
      ...
      >
        <defs>
          <linearGradient id={gradientId} x1="0" y1="0" x2="1" y2="0">
            ...
            <stop offset="34%" stopColor="currentColor" stopOpacity="0.68" />
            <stop offset="50%" stopColor="currentColor" stopOpacity="1" />
            <stop offset="66%" stopColor="currentColor" stopOpacity="0.68" />
        ...
        {GEMINI_PATHS.map((path, index) => (
          <path
            ...
            opacity={PATH_OPACITY[index] * 0.3}
            filter={`url(#${blurId})`}
          />
        ))}
