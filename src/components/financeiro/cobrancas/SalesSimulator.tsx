...
export function SalesSimulator() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <section className="finance-panel rounded-[28px] border border-border/55 bg-card/78 p-6">
        ...
        <div className="mt-6 space-y-4">
          <label className="block text-xs font-black uppercase tracking-widest text-zinc-400">Valor</label>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button type="button" key={id} onClick={() => setMethod(id as SimulatorMethod)} aria-pressed={method === id} className={`min-h-11 rounded-[16px] px-4 py-3 text-xs font-black uppercase tracking-wider ${method === id ? "bg-zinc-950 text-white dark:bg-white dark:text-zinc-950" : "bg-zinc-100 text-zinc-500 dark:bg-white/[0.06]"}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <label className="flex min-h-11 items-center gap-3 text-sm font-bold text-zinc-700 dark:text-zinc-300">
            <Checkbox checked={passFees} onCheckedChange={(checked) => setPassFees(checked === true)} />
            Repassar taxas para o cliente
          </label>
          <Button className="min-h-11 rounded-[16px] bg-zinc-950 text-white dark:bg-white dark:text-zinc-950">
            <Calculator className="mr-2 h-4 w-4" />
            Simular venda
          </Button>
        </div>
      </section>
      <aside className="finance-panel rounded-[28px] border border-border/55 bg-card/78 p-6">
