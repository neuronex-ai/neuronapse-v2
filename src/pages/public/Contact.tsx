...
const CONTACT_EMAIL = PUBLIC_CONTENT.site.organization.email;
  ...
  <div className="public-lumen-page min-h-screen overflow-x-hidden bg-background text-foreground">
    ...
    <main className="px-5 pb-20 pt-24 md:px-8 md:pb-28 md:pt-[6.5rem]">
      ...
      <section className="mx-auto mt-12 grid max-w-[1100px] gap-4 md:grid-cols-2">
        {channels.map((channel) => (
          <article key={channel.title} className="public-neurox-card rounded-[30px] p-7">
            ...
            <a href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(channel.subject)}`} className="mt-7 inline-flex items-center text-[9px] font-black uppercase tracking-[0.18em] hover:underline">Enviar e-mail <ArrowRight className="ml-2 h-4 w-4" /></a>
          </article>
        ))}
      </section>
      ...
      <section className="mx-auto mt-6 grid max-w-[1100px] gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <article className="public-neurox-card rounded-[30px] p-7"><Mail className="h-6 w-6 text-muted-foreground" /><p className="mt-8 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground">Canal oficial</p><a href={`mailto:${CONTACT_EMAIL}`} className="mt-3 block text-2xl font-black tracking-normal hover:underline">{CONTACT_EMAIL}</a><p className="mt-5 text-sm font-medium leading-relaxed text-muted-foreground/70">Inclua no e-mail o assunto, a tela envolvida, o comportamento observado e, quando possível, uma captura sem dados sensíveis.</p></article>
      </section>
    </main>
    <Footer />
  </div>
